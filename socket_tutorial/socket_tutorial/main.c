...
int start_client_protocol(int tcp_or_udp)
{
	...
	*/
	...
	SOCKET openSocketHandle = socket(AF_INET, SOCK_STREAM, tcp_or_udp);
    ...
    socketAddress.sin_family = AF_INET;
    socketAddress.sin_addr   = *((struct in_addr*)*h->h_addr_list);
    ...
    boundSocketHandle = connect(openSocketHandle, (struct sockaddr*)&socketAddress, sizeof(struct sockaddr));
    if(SOCKET_ERROR == boundSocketHandle)
	{
